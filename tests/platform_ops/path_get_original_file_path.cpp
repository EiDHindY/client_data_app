...
TEST_CASE("get_original_file_path creates expected path from simple exe directory") {
...
TEST_CASE("get_original_file_path works for Windows-style path") {
    ...
    // Compare parent_path() if needed, or use .lexically_normal() for normalization
    REQUIRE(get_original_file_path(base).lexically_normal() == expected.lexically_normal());
}