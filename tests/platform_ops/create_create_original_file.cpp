...
TEST_CASE("create_original_file creates the file in expected location",
          "[create_original_file]") {
  std::filesystem::path base =
      std::filesystem::temp_directory_path() / "create_file_test_real";
  std::filesystem::remove_all(base);

  // First create the data dir to ensure it exists before creating the file.
  std::filesystem::create_directories(
      base / std::string(infrastructure_names::DATA_DIR_NAME));

  REQUIRE_NOTHROW(create_original_file(base));

  std::filesystem::path expected =
      base / std::string(infrastructure_names::DATA_DIR_NAME) /
      std::string(infrastructure_names::ORIGINAL_FILE_NAME);
...
TEST_CASE("create_original_file throws if data directory does not exist",
          "[create_original_file]") {
  std::filesystem::path base =
      std::filesystem::temp_directory_path() / "missing_dir_create_file_test";
  std::filesystem::remove_all(base);
...
TEST_CASE("create_original_file can overwrite existing file with same name",
          "[create_original_file]") {
  std::filesystem::path base =
      std::filesystem::temp_directory_path() / "overwrite_create_file_test";
  std::filesystem::remove_all(base);
  std::filesystem::create_directories(
      base / std::string(infrastructure_names::DATA_DIR_NAME));
