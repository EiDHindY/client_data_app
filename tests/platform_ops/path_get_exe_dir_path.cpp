...
// You must run these where you know your exe path -- cannot mock GetModuleFileNameW in standard C++.

// Helper to check if path ends with expected directory (for basic real-world verification)
bool path_ends_with(const std::filesystem::path& actual, const std::filesystem::path& expected_tail) {
...
TEST_CASE("get_exe_dir_path contains the running exe", "[integration]") {
    ...
    // Note: You'd need GetModuleFileNameW to get exe name too; here, check some known file in dir.
    ...
    REQUIRE(found); // Should find our exe in its own directory (works if exe is unchanged)
}
...
// Manual/Visual test for output (run to view actual value; must confirm output by inspection)
