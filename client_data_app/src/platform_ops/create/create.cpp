...
namespace platform_ops_create
{
	...
	{
		// exe_file_path: const reference to the directory containing the executable; no deep copy of the path string occurs here
		std::filesystem::path original_file_path = exe_file_path
			/ infrastructure_names::DATA_DIR_NAME
			// operator/ builds a new path by concatenating exe_file_path and the DATA_DIR_NAME constant
		...
		// Constructs an ofstream and attempts to open (or create) the file at original_file_path
		// Internally, this calls the OS create/open system call, transitioning CPU from user to kernel mode 

		if (!original_file.is_open())
			// is_open() checks if the stream has an associated file descriptor; returns false on failure 
			throw std::runtime_error(
				"Failed to create the original file: " + original_file_path.string()
			);
		// Throws an exception if the file could not be created or opened

		original_file.close();
		// Closes the file: flushes buffers, releases file descriptor back to OS, and returns CPU control to user mode
	}
